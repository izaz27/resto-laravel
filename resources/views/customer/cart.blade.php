...
{{-- MODAL PEMBAYARAN: Dibuat Fullscreen di Mobile agar mudah di-tap --}}
<div id="payment-modal" class="fixed inset-0 z-[100] hidden">
    ...
    <div class="relative min-h-screen flex items-end md:items-center justify-center p-0 md:p-4">
        <div class="relative bg-white rounded-t-3xl md:rounded-3xl shadow-2xl max-w-md w-full p-6 md:p-8 transform transition-all animate-slide-up">
            ...
            <p class="text-sm text-gray-500 mb-6 md:mb-8">Pilih cara bayar ternyaman.</p>
            
            <form action="{{ route('customer.order.process') }}" method="POST" id="payment-form">
                @csrf
                <div class="space-y-3 md:space-y-4">
                    <label class="flex items-center p-4 border-2 border-gray-100 rounded-2xl cursor-pointer has-[:checked]:border-red-600 has-[:checked]:bg-red-50 transition-all">
                        <input type="radio" name="payment_method" value="cash" class="hidden" checked>
                        <div class="w-10 h-10 md:w-12 md:h-12 bg-green-100 rounded-xl flex items-center justify-center mr-4">
                    ...
                    <label class="flex items-center p-4 border-2 border-gray-100 rounded-2xl cursor-pointer has-[:checked]:border-red-600 has-[:checked]:bg-red-50 transition-all">
                        ...
                        <div class="w-10 h-10 md:w-12 md:h-12 bg-blue-100 rounded-xl flex items-center justify-center mr-4">
                ...
                <div class="grid grid-cols-2 gap-4 mt-8">
                    ...
                    <button type="submit" class="bg-red-600 text-white font-bold py-3 md:py-4 rounded-2xl shadow-lg">Konfirmasi</button>
                </div>
            </form>
        </div>
    </div>
