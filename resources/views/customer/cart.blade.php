...
<div id="payment-modal" class="fixed inset-0 z-[100] hidden">
    ...
    <div class="relative min-h-full flex items-end md:items-center justify-center p-0 md:p-4">
        <div class="relative bg-white rounded-t-3xl md:rounded-3xl shadow-2xl max-w-md w-full p-6 md:p-8 transform transition-all animate-slide-up max-h-[90vh] flex flex-col">
            ...
            {{-- Form Wrapper dengan Scroll jika konten kepanjangan --}}
            <form action="{{ route('customer.order.process') }}" method="POST" id="payment-form" class="flex flex-col overflow-hidden">
                @csrf
                <div class="space-y-3 overflow-y-auto pr-1 flex-1 custom-scrollbar" style="max-height: 50vh;">
                    {{-- Cash Option --}}
                    <label class="group flex items-center p-4 border-2 border-gray-100 rounded-2xl cursor-pointer transition-all">
                        <input type="radio" name="payment_method" value="cash" class="peer hidden" checked>
                        <div class="w-10 h-10 md:w-12 md:h-12 bg-green-100 rounded-xl flex items-center justify-center mr-4 group-peer-checked:bg-green-200">
                            <svg class="w-6 h-6 text-green-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 9V7a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2m2 4h10a2 2 0 002-2v-6a2 2 0 00-2-2H9a2 2 0 00-2 2v6a2 2 0 002 2zm7-5a2 2 0 11-4 0 2 2 0 014 0z"></path></svg>
                        </div>
                        <div class="flex-1">
                            <span class="block font-bold text-gray-800 text-sm md:text-base">Bayar di Kasir</span>
                            <span class="text-[10px] md:text-xs text-gray-500">Tunai setelah makan</span>
                        </div>
                        {{-- Custom border logic for Tailwind 4 --}}
                        <div class="peer-checked:border-red-600 absolute inset-0 border-2 border-transparent rounded-2xl pointer-events-none"></div>
                    </label>

                    {{-- QRIS Option --}}
                    <label class="group relative flex items-center p-4 border-2 border-gray-100 rounded-2xl cursor-pointer transition-all">
                        <input type="radio" name="payment_method" value="qris" class="peer hidden">
                        <div class="w-10 h-10 md:w-12 md:h-12 bg-blue-100 rounded-xl flex items-center justify-center mr-4 group-peer-checked:bg-blue-200">
                ...
                {{-- Tombol Aksi: Dipastikan tetap di bawah --}}
                <div class="grid grid-cols-2 gap-4 mt-6 flex-shrink-0 pb-2">
                    <button type="button" onclick="closePaymentModal()" class="py-3 md:py-4 font-bold text-gray-400 hover:text-gray-600 transition">
                        Batal
                    </button>
