...
<div class="max-w-6xl mx-auto py-6 md:py-8 px-4">
    ...
    {{-- Daftar Kategori: Scroll Horizontal --}}
    ...
    {{-- Grid Menu: Card berjejer ke samping --}}
    ...
    @forelse ($menus as $menu)
        <div class="group flex flex-col bg-white rounded-xl md:rounded-2xl shadow-sm hover:shadow-xl transition-all duration-300 border border-gray-100 overflow-hidden h-full">
            ...
            {{-- 1. Container Gambar: Kunci ukuran kotak 1:1 --}}
            ...
            <div class="p-3 md:p-5 flex flex-col flex-grow">
                ...
                <p class="hidden md:block text-xs text-gray-500 mt-2 line-clamp-2">
                    {{ $menu->description ?? 'Nikmati kelezatan menu pilihan kami.' }}
                </p>
                
                {{-- 3. Action Buttons: mt-auto memastikan tombol selalu sejajar di bawah --}}
                <div class="mt-auto pt-4 flex flex-col md:flex-row items-center gap-2">
                    <button type="button" onclick="addToCart('{{ $menu->id }}')" 
                            class="w-full bg-emerald-500 text-white px-3 py-2 rounded-lg hover:bg-emerald-600 transition text-xs md:text-sm font-bold shadow-sm active:scale-95">
                        + Keranjang
                    </button>
