...
<div class="max-w-6xl mx-auto py-6 md:py-8 px-4">
    ...
    {{-- Daftar Kategori: Diubah jadi Scroll Horizontal di Mobile --}}
    <div class="flex flex-nowrap overflow-x-auto gap-2 md:gap-3 mb-8 md:mb-10 pb-2 no-scrollbar">
        <a href="{{ route('customer.menu.index') }}" 
           class="whitespace-nowrap px-5 py-2 {{ !request('category') ? 'bg-red-600 text-white' : 'bg-white text-gray-700 hover:bg-red-50' }} rounded-full text-sm font-semibold shadow-sm transition border border-transparent">
            Semua
        </a>
    ...
    {{-- Grid Menu: 2 Kolom di Mobile agar lebih ringkas --}}
    <div class="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-8 mb-24">
    @forelse ($menus as $menu)
        <div class="group bg-white rounded-xl md:rounded-2xl shadow-md md:shadow-lg overflow-hidden transition transform active:scale-95 md:hover:scale-[1.03] duration-300 border border-gray-100 flex flex-col h-full">
            ...
            <div class="h-32 md:h-52 bg-gray-200 overflow-hidden relative flex-shrink-0">
                ...
                <div class="absolute top-2 left-2 md:top-3 md:left-3">
                    <span class="text-[8px] md:text-[10px] uppercase tracking-wider font-bold text-white bg-red-600/90 backdrop-blur-sm px-2 py-0.5 md:px-3 md:py-1 rounded-md">
                        {{ $menu->category->name }}
                    </span>
                </div>
            </div>

            {{-- Body Card: Menggunakan flex-1 agar memenuhi sisa ruang --}}
            <div class="p-3 md:p-6 flex flex-col flex-1">
                
                {{-- 2. Judul Menu: Tinggi Tetap (Fixed Height untuk 2 baris) --}}
                {{-- h-10 di mobile (cukup 2 baris teks kecil), md:h-14 di desktop --}}
                <div class="h-10 md:h-14 mb-1 overflow-hidden">
                    <h3 class="text-sm md:text-xl font-bold text-gray-900 leading-tight line-clamp-2 group-hover:text-red-600 transition">
                        {{ $menu->name }}
                    </h3>
                </div>
                
                {{-- 3. Harga: Posisi Tetap --}}
                <div class="mb-3">
                    <p class="text-base md:text-2xl font-black text-red-600">
                        Rp{{ number_format($menu->price, 0, ',', '.') }}
                    </p>
                </div>
                
                {{-- Deskripsi: Tetap sembunyi di mobile --}}
                <p class="hidden md:block text-sm text-gray-500 mb-6 line-clamp-2 min-h-[40px]">
                    {{ $menu->description ?? 'Nikmati kelezatan menu pilihan kami.' }}
                </p>
                
                {{-- 4. Tombol Aksi: Otomatis di paling bawah berkat mt-auto --}}
                <div class="mt-auto pt-3 border-t border-gray-50 flex flex-col md:flex-row justify-between items-center gap-2 md:gap-4">
                    <button type="button" onclick="addToCart('{{ $menu->id }}')" 
                            class="w-full bg-green-500 text-white px-3 py-2 md:py-2.5 rounded-lg md:rounded-xl hover:bg-green-600 transition text-[16px] md:text-base font-bold shadow-sm active:scale-90">
                        + <span class="md:inline">Keranjang</span>
                    </button>

                    <a href="{{ route('customer.menu.show', $menu->slug) }}" class="flex-shrink-0 text-[16px] md:text-sm text-blue-600 hover:text-blue-800 font-bold underline underline-offset-4">
                        Detail
                    </a>
                </div> 
            </div>
        ...
        @empty
            <div class="col-span-full text-center py-12 md:py-20 bg-gray-50 rounded-3xl border-2 border-dashed border-gray-200 mx-4">
                <p class="text-gray-400 text-lg md:text-xl font-medium italic">Menu belum tersedia...</p>
            </div>
        @endforelse
    </div>
</div>

{{-- Floating Cart Button: Lebih Slim & Lebar di Mobile --}}
    ...
    <div class="max-w-md mx-auto">
        <a href="{{ route('customer.cart.index') }}" class="flex items-center justify-between bg-red-600 text-white p-4 rounded-2xl shadow-2xl hover:bg-red-700 active:scale-95 transition border-2 border-white/20">
            <div class="flex items-center">
                ...
                <span class="font-bold tracking-wide uppercase text-xs md:text-sm">Keranjang</span>
            </div>
            <span id="cart-total-price" class="font-black text-base md:text-lg italic">
...
<script>
// Fungsi JS tetap sama dengan milikmu, hanya pastikan ID-nya sesuai
