...
<div class="max-w-xl mx-auto bg-white rounded-3xl p-8 shadow-sm border border-gray-100">
    ...
    <form action="{{ route('admin.category.update', $category->id) }}" method="POST">
        ...
        <div class="mb-6">
            ...
            <input type="text" name="name" value="{{ $category->name }}" class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition" required>
            @error('name') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
        </div>

        <div class="flex gap-3">
            <button type="submit" class="flex-1 bg-blue-600 text-white py-3 rounded-xl font-black hover:bg-blue-700 transition">UPDATE</button>
